...
distr_list = [helper.time_varying_normal_params, helper.normal_params, helper.exp_poisson_params]

# distr_list = [helper.exp_poisson_params]


algo_list = {'adaptive': helper.adaptive_allocation,
             'static' : helper.static_allocation}

algo_list.update({f'bang_bang_{delta}': 
                  (lambda delta: (lambda t, state, budget_t, demand_t, M, distr_params: 
                                  helper.bang_bang_allocation(t, state, budget_t, demand_t, M, distr_params, delta)))
    ...
    for M in M_list:
        ...
        for b in b_list:
            ...
            for alg_name, algorithm in algo_list.items():
                ...
                new_data = helper.evaluate_algorithm(b, h, M, alg_name, algorithm, distr_params, time_horizon, num_iters)
